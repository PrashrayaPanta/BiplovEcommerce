...
import { ChevronDown, Heart, Menu, Search, ShoppingCartIcon, User } from "lucide-react"
      ...
      <nav className='justify-between hidden lg:flex items-center flex p-5 border-b-2 md:px-24 px-6 z-10  sticky top-0 bg-white'>
        ...
        <div className='flex gap-12'>
                ...
                <ul className='py-2'>
                  {categories.map((category, index) => (
                      <Link
                        ...
                        className={isActive(`/categories/${category.name}`) ? 'text-orange-500' : ''}
                      >
          ...
          <div className='flex gap-4'>
            ...
            <Link to="/login"> <User className='hidden sm:flex cursor-pointer'/></Link>
            <CartSheet />
          </div>
        </div>
      </nav>
