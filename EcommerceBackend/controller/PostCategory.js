...
const PostCategory = require("../model/PostCategory.js");

const postCategoryCtrl = {
  createPostCategory: asyncHandler(async (req, res) => {
    const { title } = req.body;

    await PostCategory.create({ title });

    console.log("Imnserted succesfiullyt sdjfndskjnbj");

    res.json({ message: "created succesfuly" });
  }),

  getAllPostCategory: asyncHandler(async (req, res) => {
    const postCategories = await PostCategory.find();

    res.json({ postCategories });
  }),

  deletePostCategory: asyncHandler(async (req, res) => {
    const { id } = req.params;
